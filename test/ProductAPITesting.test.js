...
test('should validate if countDiscount handles valid discount percentage', () => {
  const product = products[2]; // Assuming product with index 2 for testing

  // Provide a valid discount percentage (20%)
  const discountedPrice = countDiscount(product.price, 20);

  // Calculate the expected discounted price
  const expectedDiscountedPrice = product.price * (1 - 20 / 100);
